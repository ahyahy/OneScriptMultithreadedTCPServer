...
namespace mtcps
{
    ...
    {
        public MsMultithreadedTCPServer dll_obj;
                    ...
                    try
                    {
                        ...
                        if (bdb?.Count() > 0)
                        {
                            ...
                            try
                            {
                                ...
                                ////// Можно сразу ответить клиенту.
                                ////// Отправка должна быть асинхронной, но мы запускаем ее "в фоне".
                                ////_ = Task.Run(async () =>
                                ////{
                                ////    try
                                ////    {
                                ////        await SendAsync(clientId, $"Клиенту {clientId}, ПРИВЕТ!").ConfigureAwait(false);
                                ////        await BroadcastAsync("Всем подключенным клиентам, ПРИВЕТ! от " + clientId).ConfigureAwait(false);
                                ////    }
                                ////    catch (Exception ex)
                                ////    {
                                ////        OnServerError?.Invoke("Ошибка отправки ответа: " + ex.Message);
                                ////    }
                                ////});
                            }
                            catch (Exception ex)
    ...
    [ContextClass("МсМногопоточныйTCPСервер", "MsMultithreadedTCPServer")]
    public class MsMultithreadedTCPServer : AutoContext<MsMultithreadedTCPServer>
    {
        public MsMultithreadedTCPServer(int port)
        {
        ...
        public void SendToClient(IValue p1)
        {
